...
Sphere::Sphere(int numFacesVertical, int numFacesHorizontal)
	: scale(glm::scale(glm::vec3(5)))
	, rotation(glm::rotate(glm::pi<GLfloat>() / 2, glm::vec3(1, 0, 0)))
	, location(glm::vec3(0))
{
	model = glm::mat4(1);
	...
	for (i = 0; i < numFacesVertical; i++) {
		...
		for (j = 0; j < numFacesHorizontal; j++) {
			double leftPhi = 2 * glm::pi<double>() * (double) j / numFacesHorizontalDouble + offsetToUse;
			double rightPhi = 2 * glm::pi<double>() * (double) (j + 1.0) / numFacesHorizontalDouble + offsetToUse;
